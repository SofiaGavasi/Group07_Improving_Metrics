...
def _resolve_named_targets(
    ...
    if not raw_targets:
        raise ValueError("Class-imbalance perturbation needs at least one target class/cluster id.")
    ...
    for token in raw_targets:
        ...
        try:
            ...
            raise ValueError(
                f"Unknown class-imbalance target '{token}'. use a class name or numeric index."
            ) from exc
        if numeric < 0 or numeric >= upper_bound:
            raise ValueError(
                f"Class-imbalance target index out of range: {numeric} (valid 0..{upper_bound - 1})."
            )
...
def _apply_class_imbalance_kmeans(
    ...
    if int(kmeans_k) > label_count:
        raise ValueError(
            f"--perturb-class-imbalance-kmeans-k ({int(kmeans_k)}) cannot exceed label count ({label_count})."
        )
    ...
    def _default_cache_path() -> Path:
        ...
        return (
            base
            / "perturbation_cache"
            / f"class_imbalance_label_kmeans_{safe_dataset}_labels{label_count}_k{int(kmeans_k)}_seed{int(seed)}.npz"
        )
    ...
    if label_cluster_ids is None:
        try:
            ...
            raise ImportError(
                "scikit-learn is required to fit label-kmeans class-imbalance clusters. "
    ...
    if prepared_targets.ndim == 1:
        filtered, drop_details = _apply_class_imbalance_single_label(
...
def _apply_class_imbalance_single_label(
    fake_samples: torch.Tensor,
    reference_samples: torch.Tensor,
    reference_targets: torch.Tensor,
    class_names: list[str],
...
def _apply_class_imbalance_multi_label(
    fake_samples: torch.Tensor,
    reference_samples: torch.Tensor,
    reference_targets: torch.Tensor,
    class_names: list[str],
    ...
    if isinstance(balance, list):
        if len(balance) != len(resolved_drop):
            raise ValueError(f"List of balances ({len(balance)}) must match number of target classes ({len(resolved_drop)})")
        balances = balance
    else:
    ...
    if int(keep_indices.size) < int(min_kept):
        raise ValueError(
            "class-imbalance perturbation kept too few fake samples after dropping multi-label classes. "
            f"kept={int(keep_indices.size)} min_kept={int(min_kept)}"
        )
...
def apply_class_imbalance(
    ...
    if config["strategy"] == "kmeans":
        if real_samples is None or reference_targets is None:
            raise ValueError("kmeans class-imbalance needs real reference samples and labels.")
        return _apply_class_imbalance_kmeans(
            fake_samples=fake_samples,
    ...
    if real_samples is None or reference_targets is None:
        raise ValueError("Label-based class-imbalance needs real reference samples and labels.")
    ...
    if prepared_targets.ndim == 1:
        return _apply_class_imbalance_single_label(
            fake_samples=fake_samples,
            reference_samples=real_samples,
            reference_targets=reference_targets,
            class_names=class_names,
