...
def _resolve_real_reference_request(args: argparse.Namespace, default_image_size: int):
    # Domain-shift perturbation swaps real-reference dataset before metric evaluation.
...
def _evaluate_and_save_metrics(
    ...
    if args.verbose:
        print(
            f"[test_ddpm] evaluating metrics with paired_count={paired_count} "
            f"feature_space={args.metrics_feature_space} feature_device={metric_device}",
            flush=True,
        )
    # Metrics are computed in Inception feature space to match standard evaluation protocols.
...
def main():
    ...
    if perturbations_enabled(args):
        ...
        if args.verbose:
            print(f"[test_ddpm] generated sample tensor shape={tuple(samples.shape)}", flush=True)
