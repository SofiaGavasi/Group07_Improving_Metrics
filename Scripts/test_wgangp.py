...
def _load_real_reference(
    ...
    loader = make_default_loader(dataset_name=dataset_name, data_root=data_root, image_size=image_size)
...
def _resolve_real_reference_request(args: argparse.Namespace):
    # Domain-shift perturbation swaps the real-reference dataset/domain for evaluation.
    override = get_domain_shift_override(args)
    if override is None:
        return args.metrics_dataset, args.metrics_data_root, int(args.image_size)
...
def _evaluate_and_save_metrics(real_samples: torch.Tensor, fake_samples: torch.Tensor, out_dir: Path, args: argparse.Namespace):
    ...
    # WGAN-GP evaluation uses the same metrics contract as other test scripts.
    if args.verbose:
        print(
            f"[test_wgangp] evaluating metrics with paired_count={paired_count} "
...
def main():
    ...
    # Optionally validate critic checkpoint compatibility without requiring it for generation.
    if args.critic_checkpoint.strip():
    ...
    if args.verbose:
        print(f"[test_wgangp] generated sample tensor shape={tuple(samples.shape)}", flush=True)
        ...
        try:
            ...
            if real_samples is None or int(real_samples.shape[0]) < int(args.metrics_samples):
                ...
                real_samples = _load_real_samples(
                    ...
                    data_root=ref_root,
                    image_size=ref_image_size,
                    sample_count=int(args.metrics_samples),
                    download_if_missing=args.metrics_download_if_missing,
                    seed=int(args.seed),
                )
            else:
            ...
            if perturbation_info is not None:
                ...
                perturbation_path = out_dir / "perturbation_config.json"
                perturbation_path.write_text(json.dumps(perturbation_info, indent=2), encoding="utf-8")
            metrics = _evaluate_and_save_metrics(real_samples=real_samples, fake_samples=samples, out_dir=out_dir, args=args)
